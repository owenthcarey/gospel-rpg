...
if sys.argv[1:] not in ([], ['--inspect-road'], ['--inspect-galilee'], ['--inspect-lake'], ['--inspect-connection']):
    sys.exit('Usage: build_assets.py [--inspect-road | --inspect-galilee | --inspect-lake | --inspect-connection]')
script = 'inspect_connection.py' if '--inspect-connection' in sys.argv else 'inspect_crossing.py' if '--inspect-lake' in sys.argv else 'inspect_galilee.py' if '--inspect-galilee' in sys.argv else 'inspect_road.py' if '--inspect-road' in sys.argv else 'generate_kit.py'
