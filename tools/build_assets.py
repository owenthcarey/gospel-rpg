...
if sys.argv[1:] not in ([], ['--inspect-road'], ['--inspect-galilee'], ['--inspect-lake']):
    sys.exit('Usage: build_assets.py [--inspect-road | --inspect-galilee | --inspect-lake]')
script = 'inspect_crossing.py' if '--inspect-lake' in sys.argv else 'inspect_galilee.py' if '--inspect-galilee' in sys.argv else 'inspect_road.py' if '--inspect-road' in sys.argv else 'generate_kit.py'
